...
import { Activity, AlertTriangle, BarChart3, CheckCircle2, DatabaseZap, Download, Gauge, Layers3, LineChart as LineIcon, ListChecks, PhoneCall, RefreshCw, Search, ShieldCheck, SlidersHorizontal, Target, TrendingDown, TrendingUp, UsersRound, WalletCards } from 'lucide-react';
...
type Tab = 'command' | 'journey' | 'sources' | 'vendors' | 'ontact' | 'media' | 'commercials' | 'powerbi' | 'explorer' | 'qa';
...
const statusFor = (score: number): Status => score <= 0 ? 'empty' : score >= 90 ? 'excellent' : score >= 72 ? 'good' : score >= 50 ? 'watch' : 'critical';
...
function buildPerformanceRow(name: string, row: Row | Record<string, number>): PerformanceRow {
...
function journeyRows(totals: Record<string, number>) {
  ...
  const stages = [
    ['Spend', d.spend, 'Media investment'], ['Impressions', d.impressions, 'Paid reach proxy'], ['Clicks', d.clicks, 'Traffic action'], ['Landing views', d.lpv, 'Page arrival'], ['Forms', d.forms, 'Lead capture'], ['Fetched', d.fetched, 'Lead ingestion'], ['Valid ID + phone', d.valid, 'Data quality'], ['Delivered', d.delivered, 'Vendor / Ontact routing'], ['Accepted', d.accepted, 'TP1 commercial result'], ['Qualified', d.qualified, 'Sales-ready volume'], ['Sales', d.sales, 'Downstream conversion'], ['Activations', d.activations, 'Fulfilled result']
  ].filter((stage) => n(stage[1]) > 0);
...
function qaRows(results: AnalyticsResult[], analytics: Analytics, totals: Record<string, number>) {
  const d = buildDerived(totals);
  const checks = [
    { check: 'Live/snapshot source health', status: results.every((r) => r.ok || r.fallbackUsed) ? 'good' : 'watch', detail: results.map((r) => `${r.source}: ${r.fallbackUsed ? 'snapshot fallback' : r.ok ? 'live/snapshot ready' : r.error || 'attention'}`).join(' | ') },
    { check: 'Metric registry coverage', status: analytics.fieldCatalog.length ? 'good' : 'critical', detail: `${number.format(analytics.fieldCatalog.length)} parameters profiled and grouped.` },
    ...
    { check: 'PII redaction', status: analytics.records.some((r) => Object.values(r).includes('[redacted]')) ? 'good' : 'watch', detail: 'Lead names, phones, emails and ID-like values are redacted in row explorer samples.' },
    ...
    { check: 'Commercial source separation', status: 'good', detail: 'Power BI activations are shown as operational reference and are not double-counted in base revenue unless assumptions are reviewed.' },
    { check: 'Funnel monotonicity', status: d.accepted > d.fetched && d.fetched > 0 ? 'watch' : 'good', detail: d.accepted > d.fetched && d.fetched > 0 ? 'Accepted leads exceed fetched leads for the selected scope; review source/date alignment.' : 'Core accepted/fetched relationship is within expected bounds.' },
    { check: 'Cloudflare resource safety', status: results.some((r) => r.truncated) ? 'watch' : 'good', detail: results.some((r) => r.truncated) ? 'At least one source response was capped; narrow the date range for full detail.' : 'No truncation flag detected.' }
  ];
...
function Card({ title, value, sub, icon: Icon }: { title: string; value: string; sub: string; icon: typeof Activity }) { return <section className="card stat"><div className="stat-icon"><Icon size={18}/></div><div><p>{title}</p><strong>{value}</strong><span>{sub}</span></div></section>; }
function Panel({ title, sub, children, action }: { title: string; sub?: string; children: React.ReactNode; action?: React.ReactNode }) { return <section className="card panel"><div className="panel-head"><div><h2>{title}</h2>{sub && <p>{sub}</p>}</div>{action}</div>{children}</section>; }
...
function App() {
  ...
  const loadSnapshot = async () => {
    ...
    const data: unknown = await response.json();
    if (!isPayload(data)) throw new Error('Attached snapshot could not be loaded.');
    return data;
  };
  const fallbackMerge = (live: Payload, fallback: Payload | null, targets: AtomicSource[]) => {
    if (!fallback) return live;
    const liveMap = new Map(live.results.map((result) => [result.source, result]));
    const snapshotMap = new Map(fallback.results.map((result) => [result.source, result]));
    const merged = targets.map((target) => {
      const current = liveMap.get(target);
      if (current?.ok && current.analytics) return current;
      const snap = snapshotMap.get(target);
      return snap ? { ...snap, ok: false, configured: current?.configured ?? false, status: current?.status, fallbackUsed: true, error: current?.error || `${target} live sync unavailable; attached snapshot displayed.` } : current;
    }).filter(Boolean) as AnalyticsResult[];
    return { ...live, ok: merged.every((result) => result.ok), results: merged };
  };
  const load = async (nextSource: ApiSource = source, fallback: Payload | null = snapshot) => {
    ...
    try {
      ...
      const live: Payload = { ok: responses.every((response) => response.ok), mode: nextSource === 'unified' ? 'client-side-unified-live-sync' : responses[0]?.mode ?? 'single-source', generatedAt: new Date().toISOString(), results: responses.flatMap((response) => response.results) };
      const combined = fallbackMerge(live, fallback, targets);
      setPayload(combined);
      if (!combined.ok) setError(`${combined.results.filter((result) => !result.ok).map((result) => `${result.source}: ${result.error || 'needs attention'}`).join(' | ')} Snapshot fallback remains visible.`);
    } catch (e) {
      const message = e instanceof Error ? e.message : String(e);
      if (fallback) setPayload(fallback);
      setError(`Live sync unavailable: ${message.includes("Unexpected token '<'") ? 'Pages Functions are not available in plain Vite dev mode' : message}. Snapshot data is displayed.`);
    } finally { setLoading(false); }
  };
  useEffect(() => { let active = true; (async () => { try { const snap = await loadSnapshot(); if (!active) return; setSnapshot(snap); setPayload(snap); await load('unified', snap); } catch (e) { if (active) setError(e instanceof Error ? e.message : String(e)); } })(); return () => { active = false; }; }, []);

  const results = payload?.results ?? [];
  const single = source === 'unified' ? undefined : results.find((item) => item.source === source) ?? results[0];
  const analytics = source === 'unified' ? mergeAnalytics(results) : single?.analytics ?? emptyAnalytics();
  ...
  const totalPerf = buildPerformanceRow('Total', totals);
  const qa = qaRows(results, analytics, totals);
  ...
  const agentRows = analytics.byAgent.slice(0, 20).map((row) => ({ agent: row.agent, records: row.records, calls: row.__call_records ?? row.records, talkSeconds: row.length_in_sec, avgSeconds: ratio(row.length_in_sec, row.records), mtnSales: row.MTN_Sales ?? 0, activations: row.MTN_Activated_Sales ?? row.count_activation ?? 0 }));
  ...
  const tabs: { id: Tab; label: string }[] = [{ id: 'command', label: 'Command Center' }, { id: 'journey', label: 'Journey' }, { id: 'sources', label: 'Sources' }, { id: 'vendors', label: 'Vendors' }, { id: 'ontact', label: 'OnTact' }, { id: 'media', label: 'Media' }, { id: 'commercials', label: 'Commercials' }, { id: 'powerbi', label: 'Power BI' }, { id: 'explorer', label: 'Explorer' }, { id: 'qa', label: 'QA Audit' }];

  return <main>
    <aside className="sidebar"><div className="brand"><div className="brand-mark">CIQ</div><div><b>ConvertIQ</b><span>Performance Analytics</span></div></div><nav>{tabs.map((item) => <button key={item.id} className={tab === item.id ? 'active' : ''} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav><div className="sync-panel"><ShieldCheck size={18}/><b>QA-safe dashboard</b><span>No DB required. Live API first, snapshot fallback, redacted rows, additive totals protected.</span></div></aside>
    <section className="workspace">
      <header className="hero"><div><p className="eyebrow">API sync · metric registry · commercial intelligence</p><h1>ConvertIQ Performance Command Center</h1><p>One executive dashboard for media efficiency, lead quality, vendor routing, Ontact operations, Power BI reference data, revenue assumptions and QA reconciliation.</p></div><button className="primary" onClick={() => load()} disabled={loading}><RefreshCw size={16} className={loading ? 'spin' : ''}/>{loading ? 'Syncing' : 'Sync dashboard'}</button></header>
      <section className="controls card"><SlidersHorizontal size={18}/><select value={source} onChange={(e) => { const next = e.target.value as ApiSource; setSource(next); load(next); }}><option value="unified">Unified Dashboard</option><option value="onvest">Onvest API</option><option value="ontact">Ontact API</option><option value="powerbi">Power BI QueryData</option></select><input type="date" value={from} onChange={(e) => setFrom(e.target.value)}/><input type="date" value={to} onChange={(e) => setTo(e.target.value)}/><select value={maxRows} onChange={(e) => setMaxRows(e.target.value)}><option value="1000">1,000 rows</option><option value="5000">5,000 rows</option><option value="10000">10,000 rows</option><option value="15000">15,000 rows</option></select><select value={recordLimit} onChange={(e) => setRecordLimit(e.target.value)}><option value="250">Show 250</option><option value="1000">Show 1,000</option><option value="2500">Show 2,500</option><option value="5000">Show 5,000</option></select><button onClick={() => load()}>Apply</button><span>{payload ? `Last sync: ${new Date(payload.generatedAt).toLocaleString()}` : 'Not synced yet'}</span></section>
      {error && <section className="notice">{error}</section>}
      <section className="source-grid">{results.map((item) => <section className="card source-card" key={item.source}><span className={item.ok ? 'pill ok' : 'pill warn'}>{item.fallbackUsed ? 'Snapshot fallback' : item.ok ? 'Ready' : 'Attention'}</span><h3>{item.source.toUpperCase()}</h3><p>{number.format(item.rows ?? 0)} rows · {number.format(item.analytics?.fieldCatalog.length ?? 0)} parameters {item.truncated ? '· capped' : ''}</p></section>)}<section className="card source-card"><span className="pill ok">Registry</span><h3>{number.format(additiveFields.length)}</h3><p>Additive metrics protected from identifier double-counting.</p></section></section>

      {tab === 'command' && <><section className="kpi-grid"><Card title="Revenue" value={currency.format(money.revenue)} sub="Base commercial model" icon={WalletCards}/><Card title="Gross Profit" value={currency.format(money.grossProfit)} sub={`${pct.format(money.margin)} margin`} icon={TrendingUp}/><Card title="Spend" value={currency.format(d.spend)} sub="Media cost" icon={DatabaseZap}/><Card title="Fetched Leads" value={number.format(d.fetched)} sub={`${pct.format(ratio(d.valid, d.fetched))} valid`} icon={UsersRound}/><Card title="Accepted Leads" value={number.format(d.accepted)} sub={`${currency.format(d.cpaAccepted)} CPA accepted`} icon={Gauge}/><Card title="Sales / Activations" value={`${number.format(d.sales)} / ${number.format(d.activations)}`} sub="Downstream outputs" icon={Target}/></section><section className="grid two"><Panel title="Performance trend" sub="Spend, accepted leads, sales and activations by day"><ResponsiveContainer width="100%" height={330}><AreaChart data={analytics.byDate}><CartesianGrid vertical={false}/><XAxis dataKey="date"/><YAxis/><Tooltip/><Area dataKey="Amount_Spent" name="Spend"/><Area dataKey="Accepted_Leads" name="Accepted"/><Area dataKey="MTN_Sales" name="MTN Sales"/><Area dataKey="count_activation" name="Power BI Activations"/></AreaChart></ResponsiveContainer></Panel><Panel title="Executive diagnostics" sub="Priority issues based on benchmarks"><div className="diagnostic-list">{qa.slice(0, 6).map((item) => <article className={`diagnostic ${item.status}`} key={item.check}><AlertTriangle size={16}/><div><b>{item.check}</b><span>{item.detail}</span></div><StatusBadge status={item.status}/></article>)}</div></Panel></section></>}

      {tab === 'journey' && <section className="grid two"><Panel title="Full-funnel waterfall" sub="Media to activation, using only populated stages"><ResponsiveContainer width="100%" height={460}><FunnelChart><Tooltip/><Funnel dataKey="value" data={journey} isAnimationActive><LabelList position="right" fill="#17202a" stroke="none" dataKey="stage"/></Funnel></FunnelChart></ResponsiveContainer></Panel><Panel title="Leakage audit" sub="Step rate and lost volume"><SimpleTable rows={journey.map((row) => ({ stage: row.stage, value: number.format(row.value), stepRate: pct.format(row.rate), lost: number.format(row.lost), signal: row.note }))}/></Panel></section>}

      {tab === 'sources' && <><section className="grid two"><Panel title="Source score ranking" sub="Composite score from validation, delivery, acceptance, sales and activation"><ResponsiveContainer width="100%" height={360}><BarChart data={sourceRows.slice(0, 14)}><CartesianGrid vertical={false}/><XAxis dataKey="name"/><YAxis domain={[0,100]}/><Tooltip/><Bar dataKey="score" name="Score"/></BarChart></ResponsiveContainer></Panel><Panel title="Source economics" sub="Spend and accepted lead efficiency"><ResponsiveContainer width="100%" height={360}><BarChart data={sourceRows.slice(0, 14)}><CartesianGrid vertical={false}/><XAxis dataKey="name"/><YAxis/><Tooltip/><Bar dataKey="spend" name="Spend"/><Bar dataKey="accepted" name="Accepted"/></BarChart></ResponsiveContainer></Panel></section><Panel title="Source performance matrix" action={<button className="secondary" onClick={() => csvDownload('convertiq-source-performance.csv', sourceRows)}>Export CSV</button>}><SimpleTable rows={sourceRows.map((row) => ({ source: row.name, score: decimal.format(row.score), spend: currency.format(row.spend), fetched: number.format(row.fetched), validRate: pct.format(row.validationRate), deliveryRate: pct.format(row.deliveryRate), accepted: number.format(row.accepted), cpaAccepted: row.cpaAccepted ? currency.format(row.cpaAccepted) : '-', sales: number.format(row.sales), activationRate: pct.format(row.activationRate) }))}/></Panel></>}
      ...
      {tab === 'powerbi' && <section className="grid two"><Panel title="Power BI QueryData summary" sub="Used as data reference, not embedded report"><SimpleTable rows={[{ metric: 'Rows', value: number.format(powerBiRows.length) }, { metric: 'Activations reference', value: number.format(Math.max(n(totals.count_activation), n(totals.total_activations))) }, { metric: 'Capture complete reference', value: number.format(Math.max(n(totals.count_capture_complete), n(totals.total_capture_complete))) }, { metric: 'Nett apps reference', value: number.format(Math.max(n(totals.count_nett_app), n(totals.total_nett_apps))) }, { metric: 'Endpoint', value: results.find((r) => r.source === 'powerbi')?.queryDataEndpoint ?? 'snapshot' }]}/></Panel><Panel title="Power BI rows"><SimpleTable rows={powerBiRows.slice(0, 200)}/></Panel></section>}

      {tab === 'explorer' && <><Panel title="Metric registry" sub={`${number.format(filteredFields.length)} of ${number.format(analytics.fieldCatalog.length)} parameters`} action={<div className="inline-tools"><Search size={16}/><input placeholder="Search parameters" value={fieldSearch} onChange={(e) => setFieldSearch(e.target.value)}/></div>}><SimpleTable rows={filteredFields.map((field) => ({ source: field.source ?? source, parameter: field.rawField ?? field.field, group: field.group, role: field.role, type: field.type, additive: isAdditiveField(field) ? 'yes' : 'no', pii: field.pii ? 'redacted' : 'no', nonNull: field.nonNull, total: isAdditiveField(field) ? fmt(field.total, field.rawField ?? field.field) : '' }))}/></Panel><Panel title="Redacted row explorer" sub={`${number.format(filteredRecords.length)} visible records`} action={<div className="inline-tools"><Search size={16}/><input placeholder="Search rows" value={search} onChange={(e) => setSearch(e.target.value)}/><button className="secondary" onClick={() => csvDownload('convertiq-redacted-rows.csv', filteredRecords)}>Export</button></div>}><SimpleTable rows={filteredRecords} columns={analytics.columns}/></Panel></>}

      {tab === 'qa' && <section className="grid two"><Panel title="QA checks"><div className="diagnostic-list">{qa.map((item) => <article key={item.check} className={`diagnostic ${item.status}`}><ShieldCheck size={16}/><div><b>{item.check}</b><span>{item.detail}</span></div><StatusBadge status={item.status}/></article>)}</div></Panel><Panel title="Additive metric audit"><SimpleTable rows={additiveFields.slice(0, 300)}/></Panel></section>}
    </section>
  </main>;
}
