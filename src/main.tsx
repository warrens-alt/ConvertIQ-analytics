...
function qaRows(results: AnalyticsResult[], analytics: Analytics, totals: Record<string, number>) {
  ...
  const checks = [
    ...
    { check: 'PII redaction', status: analytics.records.some((r) => Object.values(r).includes('[redacted]')) ? 'good' : 'watch', detail: 'Lead names, phones, emails and ID-like values are redacted in row explorer samples where returned by API.' },
    ...
    { check: 'Commercial source separation', status: 'good', detail: 'Power BI activations are shown as an operational reference and excluded from base revenue unless assumptions are reviewed.' },
...
function App() {
  ...
  return <main>
    <aside className="sidebar"><div className="brand"><div className="brand-mark">CIQ</div><div><b>ConvertIQ</b><span>Performance Analytics</span></div></div><nav>{tabs.map((item) => <button key={item.id} className={tab === item.id ? 'active' : ''} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav><div className="sync-panel"><ShieldCheck size={18}/><b>API-only production mode</b><span>No demo data, no snapshot fallback. The dashboard only displays records returned by /api/analytics.</span></div></aside>
    <section className="workspace">
      <header className="hero"><div><p className="eyebrow">Live API sync · no demo data · commercial intelligence</p><h1>ConvertIQ Performance Command Center</h1><p>One production dashboard for media efficiency, lead quality, vendor routing, Ontact operations, Power BI reference data, revenue assumptions and QA reconciliation.</p></div><button className="primary" onClick={() => load()} disabled={loading}><RefreshCw size={16} className={loading ? 'spin' : ''}/>{loading ? 'Syncing' : 'Sync dashboard'}</button></header>
