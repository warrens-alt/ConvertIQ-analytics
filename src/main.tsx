...
function mergeAnalytics(results: AnalyticsResult[]): Analytics {
  ...
  if (available.length === 0) return emptyAnalytics();
  ...
  for (const result of available) {
    ...
    for (const [field, value] of Object.entries(analytics.totals)) {
      totals[field] = (totals[field] ?? 0) + n(value);
    }
    fields.push(...analytics.fieldCatalog.map((field) => ({
      ...field,
      source: result.source,
      rawField: field.field,
      field: `${result.source}.${field.field}`
    })));
...
function App() {
  ...
  const fetchOne = async (target: AtomicSource, mode: ApiSource): Promise<Payload> => {
    ...
    if (from && target !== 'powerbi') params.set('from', from);
    if (to && target !== 'powerbi') params.set('to', to);
  ...
  const load = async (nextSource: ApiSource = source) => {
    ...
    try {
      ...
      const combined: Payload = {
        ...
        mode: nextSource === 'unified' ? 'unified-client-side-live-sync' : responses[0]?.mode ?? 'single-source-live-sync',
        generatedAt: new Date().toISOString(),
        results: responses.flatMap((response) => response.results)
      };
      setPayload(combined);
      if (!combined.ok) {
        const failed = combined.results.filter((result) => !result.ok).map((result) => `${result.source}: ${result.error || 'needs attention'}`).join(' | ');
        setError(failed || 'One or more sources need attention.');
      }
    } catch (e) {
  ...
  const powerBiResult = results.find((item) => item.source === 'powerbi');
  ...
  const filteredFields = useMemo(() => {
    ...
    return fields.filter((field) =>
      ...
      (!q || field.field.toLowerCase().includes(q) || String(field.source ?? '').toLowerCase().includes(q) || field.group.toLowerCase().includes(q) || field.role.toLowerCase().includes(q))
    );
  ...
  const tabs: { id: Tab; label: string }[] = [
    { id: 'overview', label: 'Unified Overview' },
  ...
  const sourceTitle = isUnified ? 'Unified ConvertIQ Analytics Dashboard' : source === 'onvest' ? 'Onvest Dashboard API' : source === 'ontact' ? 'Ontact Analytics API' : 'Power BI Embedded Report';
  const sourceCopy = isUnified
    ? 'One command-center view combining Onvest funnel/media metrics, Ontact call-centre records, and the Power BI visual reference layer. Sources are still fetched separately to avoid Cloudflare 1102 limits, then unified in the browser.'
    : isPowerBi
      ? 'Embedded Power BI report from the Rubix Reports production capture. Use this as the visual reference layer alongside the live Onvest and Ontact API data.'
  ...
  return <main>
    <aside className="sidebar">
      ...
      <div className="sync-panel"><ShieldCheck size={18}/><b>Unified source layer</b><span>Onvest, Ontact and Power BI are shown as one dashboard while remaining resource-safe.</span></div>
    </aside>

    <section className="workspace">
      <header className="topbar">
        <div>
          <p className="eyebrow">Unified live analytics · source registry</p>
          <h1>{sourceTitle}</h1>
          <p className="subcopy">{sourceCopy}</p>
        </div>
        <button className="primary" onClick={() => load()} disabled={loading}><RefreshCw size={16} className={loading ? 'spin' : ''}/> {loading ? 'Syncing' : isPowerBi ? 'Load report' : 'Sync dashboard'}</button>
      </header>

      <section className="controls card">
        <SlidersHorizontal size={18}/>
        <select value={source} onChange={(e) => { const next = e.target.value as ApiSource; setSource(next); if (next === 'powerbi') setTab('powerbi'); if (tab === 'powerbi' && next !== 'powerbi') setTab('overview'); load(next); }}><option value="unified">Unified Dashboard</option><option value="onvest">Onvest Dashboard API</option><option value="ontact">Ontact Analytics API</option><option value="powerbi">Power BI Report</option></select>
      ...
      {results.some((item) => item.defaultWindowApplied) && !isPowerBi && <section className="notice soft">No date range was selected, so each API request used a safe recent window. Select dates to inspect a specific period.</section>}
      ...
      <section className="source-grid">
        {results.map((item) => <section className="card source-card" key={item.source}><span className={item.ok ? 'pill ok' : 'pill warn'}>{item.ok ? 'Connected' : 'Attention'}</span><h3>{item.source.toUpperCase()}</h3><p>{item.source === 'powerbi' ? 'Embedded Power BI visual layer.' : item.ok ? `${number.format(item.rows ?? 0)} rows processed · ${number.format(item.upstreamCount ?? item.rows ?? 0)} upstream rows` : item.error || 'Waiting for sync'}</p></section>)}
        <section className="card source-card"><span className="pill ok">Unified Registry</span><h3>{number.format(fields.length)}</h3><p>{number.format(fields.filter((field) => field.numeric).length)} numeric · {number.format(fields.filter((field) => field.pii).length)} redacted sensitive fields</p></section>
        <section className="card source-card"><span className="pill ok">Unified Rows</span><h3>{number.format(records.length)}</h3><p>Sanitised records combined across API sources.</p></section>
      </section>

      {tab === 'overview' && <>
        <section className="kpi-grid">
          <StatCard title="Media Spend" value={currency.format(n(derived.spend))} sub="Onvest Amount_Spent" icon={DatabaseZap}/>
          <StatCard title="Fetched Leads" value={number.format(n(derived.fetchedLeads))} sub="Onvest Fetched_Leads" icon={UsersRound}/>
      ...
      {tab === 'funnel' && <section className="grid two">
        <ChartCard title="Unified journey waterfall"><ResponsiveContainer width="100%" height={420}><FunnelChart><Tooltip/><Funnel dataKey="value" data={funnel} isAnimationActive><LabelList position="right" fill="#173f35" stroke="none" dataKey="name"/></Funnel></FunnelChart></ResponsiveContainer></ChartCard>
      ...
      {tab === 'operations' && <section className="grid two">
        <ChartCard title="Ontact agent productivity"><ResponsiveContainer width="100%" height={420}><BarChart data={analytics.byAgent}><CartesianGrid vertical={false}/><XAxis dataKey="agent"/><YAxis/><Tooltip/><Bar dataKey="records" name="Records / Calls"/><Bar dataKey="length_in_sec" name="Talk seconds"/></BarChart></ResponsiveContainer></ChartCard>
        <DataTable rows={analytics.byStatus} title="Status / outcome breakdown"/>
      </section>}

      {tab === 'powerbi' && <section className="card powerbi-card wide">
        <div className="section-head"><div><h2>{powerBiResult?.reportTitle || result?.reportTitle || 'Power BI Report'}</h2><p>Embedded visual reference report from the Rubix Reports / Power BI capture.</p></div><MonitorUp size={24}/></div>
        {powerBiResult?.embedUrl || result?.embedUrl ? <iframe title="Power BI report" src={powerBiResult?.embedUrl || result?.embedUrl} allowFullScreen /> : <div className="notice">No Power BI embed URL configured.</div>}
      </section>}
    </section>
  </main>;
}
